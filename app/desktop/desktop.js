...
function loadNextBatch() {
  ...
  batch.forEach((ch) => {
    ...
    translateText(ch.name).then((translated) => {
      channelNameNode.textContent = translated;
    });
...
function renderFavorites() {
  ...
  favorites.forEach((c) => {
    ...
    translateText(c.name).then((translated) => {
      nameNode.textContent = translated;
    });
...
function renderLastWatched() {
  ...
  translateText(last.name).then((translated) => {
    nameNode.textContent = translated;
  });
...
async function translateText(text) {
  ...
  try {
    const response = await fetch(
      `https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=${selectedLanguage}&dt=t&q=${encodeURIComponent(text)}`
    );
